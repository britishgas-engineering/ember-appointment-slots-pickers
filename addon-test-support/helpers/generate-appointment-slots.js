...
export function generateAppointmentSlots(options) {
  ...
  const custom = Object.assign(defaults, options);
  ...
  run(() => {
    ...
    const availableAppointmentSlots = appointmentSlots.filterBy(
      'available',
      true
    );
    ...
    const firstSlot =
      ...
      moment(availableAppointmentSlots[0].get('slotPickerDay'));
    this.set('firstSlot', firstSlot);
    const firstMonth = firstSlot && firstSlot.format('M');
    const availableSlotsOfFirstMonth = availableAppointmentSlots.filter(
      (slot) => {
        return moment(slot.get('slotPickerDay')).format('M') === firstMonth;
      }
    );
    const availableDaysOfFirstMonth = availableSlotsOfFirstMonth
      .map((slot) => {
    ...
    const availableDays = availableAppointmentSlots
      .map((slot) => {
        return moment(slot.get('slotPickerDay')).format('D');
      })
      .uniq();

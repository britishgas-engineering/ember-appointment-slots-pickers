...
module(
  'Integration | Component | horizontal-list-swiper/sly',
  function (hooks) {
    ...
    test('initializes sly', async function (assert) {
      ...
      await render(hbs`
      <HorizontalListSwiper::Sly @window={{this.window}} />
    `);
    ...
    test('centers element', async function (assert) {
      ...
      await render(hbs`
      <HorizontalListSwiper::Sly @window={{this.window}} @indexUpdate={{this.indexUpdate}} @itemsUpdate={{this.items}} as |item|>
        {{item}}
      </HorizontalListSwiper::Sly>
    `);
      assert.ok(
        sly.toCenter.calledWith(0, true),
        'Center on selected element index on first render without animation'
      );
      this.set('indexUpdate', 1);
      assert.ok(
        sly.toCenter.calledWith(1, false),
        'Center on selected element index when provided with animation'
      );
      sly.toCenter = sinon.stub();
      this.set('indexUpdate', 1);
      assert.notOk(
        sly.toCenter.called,
        'Do not attempt to center if the element index is unchanged'
      );
    });
    ...
    `);
      assert
        .dom('.scroll-header-sly-item')
        .exists({ count: 3 }, '3 items shown');
      this.set('items', [1, 2, 3, 4]);
      assert.ok(sly.reload.called, 'sly reload has been called');
      assert
        .dom('.scroll-header-sly-item')
        .exists({ count: 4 }, '4 items shown');
    });
    ...
    test('reacts to move events', async function (assert) {
      ...
      await render(hbs`
      <HorizontalListSwiper::Sly @window={{this.window}} @onmove={{this.onmove}} @onmoveend={{this.onmoveend}} />
    `);
      const slyOnMoveCallback = sly.on.args.find((arg) => arg[0] === 'move')[1];
      const slyOnMoveEndCallback = sly.on.args.find(
        (arg) => arg[0] === 'moveEnd'
      )[1];
      run(() => {
        slyOnMoveCallback();
      ...
      return settled()
        .then(() => {
          ...
          assert.notOk(
            onmoveend.called,
            'onmoveend action is not triggered when sly moves'
          );
          run(() => {
        ...
        .then(() => {
          assert.ok(
            onmoveend.called,
            'onmoveend action is triggered when sly moves end'
          );
        });
    });
    ...
    test('reacts to activate events', async function (assert) {
      ...
      await render(hbs`
      <HorizontalListSwiper::Sly @window={{this.window}} @onactive={{this.onactive}} />
    `);
      const slyOnActiveCallback = sly.on.args.find(
        (arg) => arg[0] === 'active'
      )[1];
      run(() => {
        slyOnActiveCallback();
      ...
      assert.ok(
        onactive.calledWith('activeItem'),
        'onactive action is triggered when sly moves'
      );
    });
  }
);
